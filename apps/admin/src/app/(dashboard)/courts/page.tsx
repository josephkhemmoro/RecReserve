...
const EMPTY_FORM = { name: "", sport: "tennis" as Sport };
...
export default function CourtsPage() {
  ...
  const startEdit = (court: Court) => {
    ...
    setForm({ name: court.name, sport: court.sport });
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button onClick={() => { setShowForm(false); setEditingId(null); }} className="px-4 py-2 text-slate-600 text-sm font-medium rounded-lg border border-slate-300 hover:bg-slate-50">
              Cancel
            </button>
          </div>
        </div>
...
function AvailabilityPanel({
  ...
  const handleApplyToAll = async () => {
    ...
    try {
      ...
      // Delete all existing for this court
      await supabase
        .from("court_availability")
        .delete()
  ...
  const handleApplyWeekdays = async () => {
    ...
    try {
      ...
      // Delete weekday entries (Mon-Fri = 1-5)
      for (let d = 1; d <= 5; d++) {
...
function DayRowEditor({
  ...
  return (
    <div className={`flex items-center gap-4 px-6 py-3.5 ${existing ? "" : "opacity-60"}`}>
      {/* Toggle */}
      <button
      ...
      {enabled ? (
        <>
          <input
            type="time"
            value={open}
            onChange={(e) => { setOpen(e.target.value); setDirty(true); }}
          ...
          <input
            type="time"
            value={close}
            onChange={(e) => { setClose(e.target.value); setDirty(true); }}
            className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
