...
export function SlotDemandIndicator({ demandLevel, compact = false }) {
  ...
  if (compact) return <View style={[styles.dot, { backgroundColor: color }]} />

  return (
    <View style={styles.full}>
      <View style={[styles.dot, { backgroundColor: color }]} />
      <Text style={[styles.label, { color }]}>{DEMAND_LABELS[demandLevel] || 'Open'}</Text>
    </View>
  )
}
...
const styles = StyleSheet.create({
  ...
  full: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs },
